...
public:
	...
	int getCusto(int tempo){
		...
		else if (tempo == horaPreferencial){ return custoAterrAntecipada*(horaPreferencial - tempo); } //poe 0
		else if (tempo < horaPreferencial) { return custoAterrAntecipada*(horaPreferencial - tempo); }
		else { return custoAterrRetardada*(tempo - horaPreferencial); } //erro aqui é custoAterrRetardada
	}

	friend ostream& operator<<(std::ostream& os, const Aviao* obj)
	{
		os << "{" << obj->nome << ": [" << obj->horaJanelaInicio << " | " << obj->horaPreferencial << " | " << obj->horaJanelaFim << "]}";
		return os;
	}
};
